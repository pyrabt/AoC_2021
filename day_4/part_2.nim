import sequtils, strutils, sugar, math, times
...
for call in bingoCalls:
    ...
    while checkForWinner(bingoCards) != -1 and bingoCards.len != 0:
        var winnerCheck = checkForWinner(bingoCards)
        lastWinner = bingoCards[winnerCheck]
        winningCall = parseInt(call)
        bingoCards.del(winnerCheck)
echo "Losing Card has a score of: ", winningCall*getUncalledSum(lastWinner)